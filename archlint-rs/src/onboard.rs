...
pub struct OnboardResult {
    ...
    /// Detected vendor groups (group -> fragments).
...
/// Known Go vendor groups: group name -> list of import path fragments.
/// These are used both for auto-detection (scanning go.sum / go.mod) and
/// as the default suggestion set in the generated config.
fn known_go_vendor_groups() -> Vec<(&'static str, Vec<&'static str>)> {
    vec![
        ("web-framework", vec!["gin", "echo", "chi", "fiber"]),
        ("database",      vec!["gorm", "sqlx", "pgx", "database/sql"]),
        ("logging",       vec!["zap", "zerolog", "slog"]),
        ("grpc",          vec!["grpc", "connect-go", "twirp"]),
        ("messaging",     vec!["nats", "kafka", "amqp", "rabbitmq"]),
        ("cache",         vec!["redis", "memcache", "ristretto"]),
        ("tracing",       vec!["opentelemetry", "jaeger", "zipkin"]),
    ]
}

/// Detect which vendor groups are likely used in the project by scanning
/// `go.mod` / `go.sum` for known dependency fragments.
///
/// Returns a HashMap of group_name -> list_of_fragments for groups that
/// have at least one matching import in the manifest files.  If no manifest
/// is found, returns an empty map so the caller can decide what to do.
pub fn detect_go_vendors(dir: &Path) -> HashMap<String, Vec<String>> {
    // Try go.mod first (always present in Go projects), fall back to go.sum.
    let content = {
        let mod_path = dir.join("go.mod");
        let sum_path = dir.join("go.sum");
        let from_mod = std::fs::read_to_string(&mod_path).unwrap_or_default();
        let from_sum = std::fs::read_to_string(&sum_path).unwrap_or_default();
        format!("{}\n{}", from_mod, from_sum)
    };

    let mut detected: HashMap<String, Vec<String>> = HashMap::new();
    for (group, fragments) in known_go_vendor_groups() {
        for frag in &fragments {
            if content.contains(frag) {
                detected
                    ...
                    .or_default()
                    .push(frag.to_string());
                // Keep going: collect all matching fragments for this group.
            }
        }
    }
    detected
...
fn generate_yaml(
    ...
    vendors: &HashMap<String, Vec<String>>,
) -> String {
    ...
    // Emit vendors section if any groups were detected.
    if !vendors.is_empty() {
        out.push_str("\n# Vendor/dependency grouping\n");
        out.push_str("# Group third-party imports by concern to reduce noise in the dependency graph.\n");
        out.push_str("# Each entry: group-name -> list of import path fragments to match.\n");
        out.push_str("vendors:\n");
        let mut sorted_vendor_keys: Vec<&String> = vendors.keys().collect();
        sorted_vendor_keys.sort();
        for key in sorted_vendor_keys {
            let fragments = &vendors[key];
            out.push_str(&format!("  {}: [{}]\n", key, fragments.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ")));
        }
    }
...
pub fn onboard(dir: &Path) -> OnboardResult {
    ...
    // Detect vendor groups for Go projects by scanning go.mod / go.sum.
    let vendors: HashMap<String, Vec<String>> = if languages.contains(&Language::Go) {
        detect_go_vendors(dir)
    } else {
        HashMap::new()
    };

    let config_yaml = generate_yaml(&languages, &layout, &layers, &allowed, &vendors);
...
mod tests {
    ...
    #[test]
    fn test_detect_go_vendors_empty_when_no_go_mod() {
        let dir = TempDir::new().unwrap();
        let vendors = detect_go_vendors(dir.path());
        assert!(vendors.is_empty());
    }

    #[test]
    fn test_detect_go_vendors_gin_in_go_mod() {
        let dir = TempDir::new().unwrap();
        write_file(
            &dir,
            "go.mod",
        ...
        let vendors = detect_go_vendors(dir.path());
        assert!(vendors.contains_key("web-framework"), "expected web-framework group");
        let frags = vendors.get("web-framework").unwrap();
        assert!(frags.contains(&"gin".to_string()));
    }

    #[test]
    fn test_detect_go_vendors_gorm_and_zap() {
        let dir = TempDir::new().unwrap();
        write_file(
            &dir,
            "go.mod",
        ...
        let vendors = detect_go_vendors(dir.path());
        assert!(vendors.contains_key("database"), "expected database group");
        assert!(vendors.contains_key("logging"), "expected logging group");
    }

    #[test]
    fn test_detect_go_vendors_no_known_deps() {
        let dir = TempDir::new().unwrap();
        write_file(
            &dir,
            "go.mod",
        ...
        let vendors = detect_go_vendors(dir.path());
        // testify is not in our known list
        assert!(vendors.is_empty());
    }

    #[test]
    fn test_detect_go_vendors_from_go_sum_fallback() {
        let dir = TempDir::new().unwrap();
        // Write a minimal go.mod without deps (no vendor in require block),
        // but go.sum references zerolog.
        write_file(&dir, "go.mod", "module example.com/myapp\n\ngo 1.21\n");
        write_file(
            &dir,
            "go.sum",
            "github.com/rs/zerolog v1.32.0 h1:xxx\n",
        );
        let vendors = detect_go_vendors(dir.path());
        assert!(vendors.contains_key("logging"), "expected logging from go.sum");
    }

    #[test]
    fn test_onboard_includes_vendors_for_go_project() {
        let dir = TempDir::new().unwrap();
        write_file(
            &dir,
            "go.mod",
        ...
        let result = onboard(dir.path());
        assert!(result.vendors.contains_key("web-framework"));
        assert!(result.vendors.contains_key("database"));
        assert!(result.config_yaml.contains("vendors:"));
        assert!(result.config_yaml.contains("web-framework:"));
        assert!(result.config_yaml.contains("database:"));
    }

    #[test]
    fn test_onboard_no_vendors_for_rust_project() {
        let dir = TempDir::new().unwrap();
        mk(&dir, "Cargo.toml");
        mk(&dir, "src/main.rs");
        let result = onboard(dir.path());
        // Vendor detection is only for Go currently.
        assert!(result.vendors.is_empty());
        assert!(!result.config_yaml.contains("vendors:"));
    }

    #[test]
    fn test_yaml_vendors_section_is_valid_yaml() {
        let dir = TempDir::new().unwrap();
        write_file(
            &dir,
            "go.mod",
        ...
        let result = onboard(dir.path());
        let parsed: Result<serde_yaml::Value, _> = serde_yaml::from_str(&result.config_yaml);
        assert!(parsed.is_ok(), "generated YAML with vendors is invalid: {:?}", parsed.err());
    }
}
