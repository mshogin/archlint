...
<!--
Norms - стандарты команды и проекта, которым должна следовать реализация.
Не функциональные требования, а правила оформления / observability / стиля.

Для XS/S: можно опустить, если нет специфики поверх дефолта проекта
Для M/L/XL: явно перечислить применимые нормы

Часть норм автоматизируется (golangci-lint, archlint правила) -
ссылаться на конкретный rule, а не дублировать текст.
-->

- N1: [Norm 1] - [например: error wrapping через fmt.Errorf("%w") или errtrace.Wrap, голое return err не допускается]
- N2: [Norm 2] - [например: логирование через slog с контекстом, без глобальных loggerов]
- N3: [Norm 3] - [например: naming - DB snake_case, JSON camelCase]

Автоматизировано:
- [ ] golangci-lint passes (включая errcheck, errorlint, sloglint)
- [ ] archlint passes (правила: layered, fan-out, ISP)
...
<!--
Safeguards - неприкосновенные инварианты. Нарушение = стоп, не warning.
Это то, что НЕ должно сломаться никогда вне зависимости от того,
что меняется в реализации.

Для XS: обычно не нужно
Для S: 1-2 пункта, если задача касается публичного контракта
Для M/L/XL: обязательный раздел
-->

- S1: [Safeguard 1] - [например: обратная совместимость публичного API в рамках minor релиза]
- S2: [Safeguard 2] - [например: p99 latency endpoint X не растёт более чем на 10%]
- S3: [Safeguard 3] - [например: security - нет утечки PII в логи и метрики]

Проверка:
- [ ] Smoke test на обратную совместимость
- [ ] Бенчмарк до/после с фиксацией результата
