...
// Golden layer_traversal (порт validate_layer_traversal) vs Python: контекст
// [service, cmd, repository] -> 1 нарушение (service->cmd, подъём 2->0).
func TestLayerTraversal_vsPython(t *testing.T) {
	cfg := &archlintcfg.Config{Contexts: []archlintcfg.ContextDef{
		{Name: "flow", Components: []string{"app/service/X", "app/cmd/Y", "app/repository/Z"}},
	}}

	v := ComputeLayerTraversal(cfg)
	if len(v) != 1 {
		t.Fatalf("ожидалось 1 нарушение (service->cmd), got %d: %+v", len(v), v)
	}
	if v[0].From != "app/service/X" || v[0].To != "app/cmd/Y" || v[0].FromLayer != 2 || v[0].ToLayer != 0 {
		t.Fatalf("неверное нарушение: %+v", v[0])
	}
}

// ★INTENT-LADEN ДЕМОНСТРАЦИЯ: ТЕ ЖЕ компоненты в ОБРАТНОМ порядке списка -> 0 нарушений.
// Вердикт зависит от ПОРЯДКА в context-декларации (артефакт, не поток) -> intent-laden.
func TestLayerTraversal_OrderArtifact(t *testing.T) {
	up := &archlintcfg.Config{Contexts: []archlintcfg.ContextDef{
		{Name: "c", Components: []string{"app/model/A", "app/cmd/B"}}, // model(3)->cmd(0): подъём -> 1
	}}
	down := &archlintcfg.Config{Contexts: []archlintcfg.ContextDef{
		{Name: "c", Components: []string{"app/cmd/B", "app/model/A"}}, // cmd(0)->model(3): спуск -> 0
	}}

	if len(ComputeLayerTraversal(up)) != 1 {
		t.Errorf("model->cmd (подъём) ожидался 1")
	}
	if len(ComputeLayerTraversal(down)) != 0 {
		t.Errorf("cmd->model (спуск) ожидался 0 — но те же компоненты! (order-артефакт)")
	}
}

// Без контекстов -> неактивен.
func TestLayerTraversal_Inactive(t *testing.T) {
	if v := ComputeLayerTraversal(&archlintcfg.Config{}); v != nil {
		t.Errorf("без contexts -> nil, got %+v", v)
	}
}
